...
// would then leave visitors with no service worker at all.
const PRECACHE = [
  '/app',
  '/images/Bharat%20AI%20Innovation%20Logo.png',
  '/images/icon-192.png',
  '/manifest.webmanifest',
  '/js/pass-render.js',        // /app's only same-origin script
  '/css/style.css',            // marketing shell: /, /conference, /exhibition, ...
  '/js/main.js',
  '/js/carousel.js',
];

// Assets from the self-hosting workstream (Tailwind compiled at build time,
// Font Awesome subset). Warmed separately and best-effort ON PURPOSE: these
// are generated by the build, and a single 404 inside addAll would abort the
// install permanently rather than cost us one uncached file.
//
// The ?v= must match what the pages actually request (TW_CSS / FA_CSS in
// src/index.tsx): the fetch handler below matches on the full URL, so a
...
const PRECACHE_OPTIONAL = [
  ...
  '/webfonts/fa-regular-400.woff2',   // the hollow half of the marketplace rating stars
];
  ...
  event.waitUntil(
    caches.open(SHELL)
      .then((c) => c.addAll(PRECACHE).then(() =>
        Promise.all(PRECACHE_OPTIONAL.map((u) => c.add(u).catch(() => {})))
      ))
